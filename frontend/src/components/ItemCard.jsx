...
const ItemCard = ({ id, name, quantity, price, image, _id }) => {
  ...
  const removeFromCart = async (id) => {
    const res = await axios.delete(
      `https://foodapp-o963.onrender.com/api/remove-from-cart/${id}`
    );
  ...
  const incrementQuantity = async (id) => {
    const res = await axios.put(
      `https://foodapp-o963.onrender.com/api/increment-quantity/${id}`
    );
    const data = await res.data;
    getCart(user).then((data) => dispatch(setCart(data.cartItems)));
  };
  ...
  const decrementQuantity = async (id) => {
    const res = await axios.put(
      `https://foodapp-o963.onrender.com/api/decrement-quantity/${id}`
    );
    const data = await res.data;
    getCart(user).then((data) => dispatch(setCart(data.cartItems)));
  };
